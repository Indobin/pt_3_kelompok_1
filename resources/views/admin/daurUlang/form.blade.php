            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ $page_meta['url'] }}" method="post" enctype="multipart/form-data" novalidate>
                    @method($page_meta['method'])
                        @csrf
                     <div class="mb-6">
                        <x-input-label for="name" :value="__('Foto Tempat')" />
                    ...
                    <div class="mb-6">
                        <x-input-label for="kontak_info" :value="__('Kontak_Info')" />
                        <x-text-input id="kontak_info" class="block mt-1 w-full" type="text" name="kontak_info"/>
                        <x-input-error :messages="$errors->get('kontak_info')" class="mt-2" />
                    </div>
                    <div class="mb-6">
                        <x-input-label for="deskripsi" :value="__('deskripsi')" />
                        <x-text-area id="deskripsi" class="block mt-1 w-full" name="deskripsi"></x-text-area>
